...
MultiFSClientSSH.prototype.sha1 = function(p, cb) {
  if (!this.client) {
    this.queue.push(['sha1', [p, cb]])
    return
  }

  p = this.cleanPath(p)
  cb = once(cb)
  var cmd = 'shasum ' + JSON.stringify(p) + ' || ' +
            'sha1sum ' + JSON.stringify(p)

  this.debug('sha1 cmd=', cmd)
  this.connection.exec(cmd, function (er, stream) {
    if (er) {
      this.debug('sha1 exec error')
      return cb(er)
    }
    ...
    var raw = {}
    stream.on('exit', function(code, signal, dumped, desc) {
      this.debug('sha1 exit', code, signal, dumped, desc)
      if (code === null) {
        raw.signal = signal
        raw.dumped = dumped
        raw.description = desc
    ...
    stream.on('end', function() {
      raw.stdout = stdout
      raw.stderr = stderr
      this.debug('sha1 stream end', raw)
      if (raw.exitCode !== undefined || raw.signal !== undefined)
        done.call(this)
    }.bind(this))
