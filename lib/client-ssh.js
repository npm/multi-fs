...
MultiFSClientSSH.prototype.writeFile = function(p, data, enc, cb) {
  ...
  var w = this.client.createWriteStream(tmpdest)
  if (cb) {
    cb = once(cb)
    w.on('error', cb)
    w.on('close', function() { this.rename(tmpdest, p, cb)}.bind(this))
  }
...
MultiFSClientSSH.prototype.rename = function(src, dest, cb) {
  ...
  if (src.indexOf(this.path) !== 0) src = this.cleanPath(src)
  if (dest.indexOf(this.path) !== 0) dest = this.cleanPath(dest)
