...
MultiFSClientManta.prototype.writeFile = function(p, data, enc, cb) {
  // This client does not require the write-to-tmp-then-rename approach.
  p = this.cleanPath(p)
  if (typeof enc === 'function') {
    cb = enc
    enc = null
...
MultiFSClientManta.prototype.sha1 = function(fpath, callback) {

  fpath = this.cleanPath(fpath);
  var hasher = crypto.createHash('sha1');
  hasher.setEncoding('hex');

  this.client.get(fpath, function (err, readstream) {
    if (err)
      return callback(err);
...
MultiFSClientManta.prototype.rename = function(src, dest, cb) {
  src = this.cleanPath(src)
  dest = this.cleanPath(dest)

  this.client.info(src, function(er, info) {
    if (er) {
      if (er.statusCode === 404)
        er.code = 'ENOENT'
      return cb(er, null, info)
    ...
    if (info.type === 'application/x-json-stream; type=directory') {
      er = new Error('Cannot rename a directory')
      er.code = 'EISDIR'
      return cb(er, null, info)
    }

    this.client.ln(src, dest, function(err) {
      if (err) return cb(err)
      this.client.unlink(src, function(err) {
        cb(err)
      })
    }.bind(this))
  }.bind(this))
